		...
		this.registerMarkdownCodeBlockProcessor("universe-timeline-block", (source, el, ctx) => {
			...
			if (file) {
				this.app.vault.read(file).then(content => {
					let events = deconstractContent(content).filter(event => event.member.contains(keyword));
					if (events?.length == 0) {
						el.createEl("h1", { text: "events not found" })
					} else {
...
function deconstractRows(rows: string[]): Event[] {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		if (row.length == 0 && tempEvent.inited()) {
			...
			tempEvent.member.push(substringBetween(substringAfter(row, "@member:"), "[", "]"));
		} else if (row.startsWith("@tag:")) {
			tempEvent.tag.push(substringBetween(substringAfter(row, "@tag:"), "[", "]"));
